...
const SubcategoryUpdateSchema = z
  .object({
    id: z.string().min(1), // Expect 'id' which holds the "Document ID" value
...
export async function POST(request: Request) {
  try {
    ...
    console.log("Получена заявка за обновяване на подкатегория, тяло:", JSON.stringify(body, null, 2))
    ...
    const query = `UPDATE subcategories SET ${setClauses.join(", ")} WHERE "Document ID" = $${valueIndex} RETURNING "Document ID", title;`
